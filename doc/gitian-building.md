...
*Setup instructions for a gitian build of ZENZO using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ZENZO
...
Clone the git repositories for zenzo and gitian and then checkout the zenzo version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the zenzo repository
...
To build Zenzo (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the zenzo repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zenzo/.git/
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Zenzo.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit zenzo=${COMMIT} --url zenzo=${URL} ../Zenzo/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zenzo=${COMMIT} --url zenzo=${URL} ../Zenzo/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zenzo=${COMMIT} --url zenzo=${URL} ../Zenzo/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zenzo-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zenzo-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/zenzo-build.assert
```
