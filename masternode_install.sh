...
#
#	Platforms:
#               - Linux Ubuntu ONLY 16.04/18.04 LTS on a Vultr, Hetzner or DigitalOcean VPS
    ...
    for NUM in $(seq 1 ${count}); do
        ...
        if [ ! -f ${MNODE_CONF_BASE}/${CODENAME}_n${NUM}.conf ]; then
            ...
            if [ -e config/${CODENAME}/${CODENAME}.conf ]; then
                ...
                cp ${SCRIPTPATH}/doc/mn_guide/config/${CODENAME}/${CODENAME}.conf ${MNODE_CONF_BASE}/${CODENAME}_n${NUM}.conf  &>> ${SCRIPT_LOGFILE}
            else
                echo "No ${CODENAME} template found, using the default configuration template"			          &>> ${SCRIPT_LOGFILE}
                cp ${SCRIPTPATH}/doc/mn_guide/config/default.conf ${MNODE_CONF_BASE}/${CODENAME}_n${NUM}.conf                  &>> ${SCRIPT_LOGFILE}
            fi
...
function generate_privkey() {
    ...
    ${CODENAME}-cli -conf=${MNODE_CONF_BASE}/${CODENAME}_test.conf -datadir=${MNODE_DATA_BASE}/${CODENAME}_test stop
    sleep 5
...
function source_config() {
    ...
    SETUP_CONF_FILE="${SCRIPTPATH}/doc/mn_guide/config/${project}/${project}.env"
...
function print_logo() {
    ...
    if [ -f "${SCRIPTPATH}/doc/mn_guide/$CODENAME.jpg" ]; then
        jp2a -b --colors --width=56 ${SCRIPTPATH}/doc/mn_guide/${CODENAME}.jpg
    else
        jp2a -b --colors --width=56 ${SCRIPTPATH}/doc/mn_guide/default.jpg
    fi
...
function build_mn_from_source() {
    ...
    if [ ! -f ${MNODE_DAEMON} ] || [ "$update" -eq 1 ]; then
        ...
        source ${SCRIPTPATH}/doc/mn_guide/config/${CODENAME}/${CODENAME}.compile | pv -t -i0.1
    else
...
function final_call() {
    ...
    if [ "$update" -eq 0 ]; then
        cp ${SCRIPTPATH}/doc/mn_guide/scripts/activate_masternodes.sh ${MNODE_HELPER}_${CODENAME}
...
source ${SCRIPTPATH}/doc/mn_guide/config/default.env
#################################################
