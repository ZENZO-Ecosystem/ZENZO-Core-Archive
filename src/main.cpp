...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState& state, const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
    ...
    // Permanently reject new zerocoin transactions
    if (tx.ContainsZerocoins())
        return state.DoS(10, error("AcceptToMemoryPool : Zerocoin rejected"), REJECT_INVALID, "bad-tx");
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zZNZ spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zZNZ spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zZNZ spend with serial %s from tx %s is not in valid range\n",
...
{
    RenameThread("zenzo-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZZNZMinted()
{
...
bool RecalculateZNZSupply(int nHeightStart)
{
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zZNZ with serial %s is already in the block %d\n",
    ...
    if (!fVerifyingBlocks && pindex->nHeight == Params().Zerocoin_StartHeight() + 1) {
        RecalculateZZNZMinted();
        RecalculateZZNZSpent();
        RecalculateZNZSupply(1);
    }
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zZNZSpent: %s\n",
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zZNZ spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zZNZ serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    unsigned int nBitsRequired = GetNextWorkRequired(pindexPrev, &block, Params().GetConsensus());
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
   ...
   bool isPos = block.IsProofOfStake();
    if (isPos) {
        ...
        // Inputs
        std::vector<CTxIn> znzInputs;
        std::vector<CTxIn> zZNZInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zZNZInputs.push_back(stakeIn);
            }else{
                znzInputs.push_back(stakeIn);
            }
        }
        const bool hasPIVInputs = !znzInputs.empty();
        const bool hasZPIVInputs = !zZNZInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasPIVInputs)
                    ...
                    for (const CTxIn& pivIn : znzInputs){
        ...
        if (isBlockFromFork) {
            ...
            do {
                ...
                for (const CTransaction& t : bl.vtx) {
                    for (const CTxIn& in: t.vin) {
                        ...
                        for (const CTxIn& stakeIn : znzInputs) {
            ...
            // Now that this loop if completed. Check if we have zZNZ inputs.
            if(hasZPIVInputs){
                for (const CTxIn& zPivInput : zZNZInputs) {
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zZNZ mints and %d zZNZ spends\n", __func__, nMints, nSpends);
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (!pto->fDisconnect && state.vBlocksInFlight.size() > 0 && state.vBlocksInFlight.front().nTime < nNow - 500000 * chainparams.GetConsensus().nPowTargetSpacing * (4 + state.vBlocksInFlight.front().nValidatedQueuedBefore)) {
