...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the ZNZ and zZNZ money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 26210, 26211));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, zenzo, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Zenzo specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzbrk=<n>", strprintf(_("Enable automatic wallet backups triggered after each zZNZ minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizezenzoamount=<n>     " + strprintf(_("Keep N ZNZ anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 26211, 38843));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Zenzo Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // Zenzo: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZZNZBackups = GetBoolArg("-backupzbrk", true);
        pwalletMain->setZZNZAutoBackups(fEnableZZNZBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeZenzoAmount = GetArg("-anonymizezenzoamount", 0);
//    if (nAnonymizeZenzoAmount > 999999) nAnonymizeZenzoAmount = 999999;
//    if (nAnonymizeZenzoAmount < 2) nAnonymizeZenzoAmount = 2;
    ...
    LogPrintf("Anonymize Zenzo Amount %d\n", nAnonymizeZenzoAmount);
    ...
    /* Denominations
       ...
       For example:
       1ZNZ+1000 == (.1ZNZ+100)*10
       10ZNZ+10000 == (1ZNZ+1000)*10
    */
