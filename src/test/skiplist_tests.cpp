...
BOOST_AUTO_TEST_CASE(getlocator_test)
{
    ...
    for (unsigned int i=0; i<vBlocksSide.size(); i++) {
        vHashSide[i] = Ari            BOOST_CHECK_EQUAL(locator.vHave[i].GetLow64(), tip->nHeight - i);
    thToUint256(i + 50000 + (arith_uint256(1) << 128)); // Add 1<<128 to the hashes, so GetLow64() still returns the height.
