...
void ComputePassfactor(std::string ownersalt, uint256 prefactor, uint256& passfactor)
{
    ...
    arith_uint512 temp(ReverseEndianString(HexStr(prefactor) + ownersalt));
...
std::string BIP38_Encrypt(std::string strAddress, std::string strPassphrase, uint256 privKey, bool fCompressed)
{
    ...
    uint256 block1 = ArithToUint256(((UintToArith256(privKey) << 128) ^ (derivedHalf1 << 128)) >> 128);
    ...
    uint256 block2 = ArithToUint256(p2 ^ dh12);
...
bool BIP38_Decrypt(std::string strPassphrase, std::string strEncryptedKey, uint256& privKey, bool& fCompressed)
{
    ...
    //invalid prefix
    if(arith_uint256(ReverseEndianString(strKey.substr(0, 2))) != arith_uint256(0x01))
        return false;
    ...
    arith_uint256  encryptedPart1(ReverseEndianString(strKey.substr(30, 16)));
    arith_uint256  encryptedPart2(ReverseEndianString(strKey.substr(46, 32)));

    fCompressed = (flag & arith_uint256(0x20)) != 0;

    //not ec multiplied
    if (type == arith_uint256(0x42)) {
        uint512 hashed;
        encryptedPart1 = arith_uint256(ReverseEndianString(strKey.substr(14, 32)));
        uint64_t salt = uint256S(ReverseEndianString(strAddressHash)).GetCheapHash();
        scrypt_hash(strPassphrase.c_str(), strPassphrase.size(), BEGIN(salt), strAddressHash.size() / 2, BEGIN(hashed), 16384, 8, 8, 64);
        ...
        DecryptAES(ArithToUint256(encryptedPart1), derivedHalf2, decryptedPart1);

        uint256 decryptedPart2;
        DecryptAES(ArithToUint256(encryptedPart2), derivedHalf2, decryptedPart2);
        ...
        return true;
    } else if (type != arith_uint256(0x43)) //invalid type
        return false;
    ...
    uint256 decryptedPart1;
    arith_uint256 x2 = x1 & UintToArith256(uint256S("0xffffffffffffffff")); // set x2 to seedbPart1 (still encrypted)    x2 = x2 << 64;                                   //make room to add encryptedPart1 to the front
