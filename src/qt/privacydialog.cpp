                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zZNZ ought to be enough for anybody." - Bill Gates, 2017
    ui->zZNZpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzZNZSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzZNZ->setEnabled(false);
        ui->pushButtonMintzZNZ->setToolTip(tr("zZNZ is currently disabled due to maintenance."));

        ui->pushButtonSpendzZNZ->setEnabled(false);
        ui->pushButtonSpendzZNZ->setToolTip(tr("zZNZ is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zZNZpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzZNZ_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zZNZ...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zZNZ in ") +
...
void PrivacyDialog::on_pushButtonSpendzZNZ_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zZNZ is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    // Wallet already unlocked or not encrypted at all, send zZNZ
    sendzZNZ();
}

void PrivacyDialog::on_pushButtonZZNZControl_clicked()
{
    ZZNZControlDialog* zZNZControl = new ZZNZControlDialog(this);
    zZNZControl->setModel(walletModel);
    zZNZControl->exec();
}

void PrivacyDialog::setZZNZControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzZNZSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzZNZ()
{
    ...
    double dAmount = ui->zZNZpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zZNZ is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " ZNZ </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zZNZ</b>";
    ...
    if (!ZZNZControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZZNZControlDialog::GetSelectedMints();
    }
    ...
    ZZNZControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zZNZ Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " ZNZ, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zZNZ Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zZNZ </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zZNZ "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zZNZ "));
    ui->labelzZNZAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    // Display AutoMint status

    updateAutomintStatus();

    // Display global supply
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zZNZ </b> "));
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zZNZ </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzZNZSyncStatus->setVisible(fShow);
}
