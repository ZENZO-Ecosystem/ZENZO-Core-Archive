...
std::list<std::string> ZZNZControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZZNZControlDialog::listMints;

ZZNZControlDialog::ZZNZControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZZNZControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZZNZControlDialog::~ZZNZControlDialog()
{
    delete ui;
}

void ZZNZControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZZNZControlDialog::updateList()
{
...
void ZZNZControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZZNZControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZZNZ_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZZNZControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZZNZControlDialog::GetSelectedMints()
{
...
void ZZNZControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
